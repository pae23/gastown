...
func nukePolecatFull(polecatName, rigName string, mgr *polecat.Manager, r *rig.Rig) error {
	...
	// Remote branch is never deleted here — the refinery owns that (gt-v5ku).
	if branchToDelete != "" {
		var repoGit *git.Git
		bareRepoPath := filepath.Join(r.Path, ".repo.git")
		if info, statErr := os.Stat(bareRepoPath); statErr == nil && info.IsDir() {
		...
		// This prevents the race where nuke deletes the branch before
		// the refinery has a chance to merge it.
		// Fixes: gt-v5ku
